...
```{python}
# Display Element 1 : Welcome Text
```
...
Use the navigation bar to explore each section.
...
print(f"🌐 Global average CO₂ per capita in {latest_year}: {global_avg_co2} tons")
```
...
extremes.style.set_caption("Highest and Lowest CO2 Emissions Per Capita")
```
...
print(f"🌐 Projected Global population in {df_pop['year'].max()}: {global_pop/1e9:.2f} billion")
```
